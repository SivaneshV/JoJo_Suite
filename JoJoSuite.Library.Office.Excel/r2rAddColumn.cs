        ...
        public bool DoAction()
        {
            ...
            try
            {
                if (_xlTableName!="")
                {
                    ...
                    for (int i = 1; i <= ListCount; i++)
                    {
                        if (_xlTableName.ToString().ToLower() == _xlWorksheet.ListObjects[i].Name.ToString().ToLower())
                        {
                            ...
                            {
                                _xlWorksheet.ListObjects[i].ListColumns.Add(_xlColumnNoOrName);
                            }
                            break;
                        }
                    }
                ...
                else
                {
                    for (int j = 1; j <= _xlCount; j++)
                    {
                        _xlWorksheet.Range[_xlColumnNoOrName].EntireColumn.Insert(XlInsertShiftDirection.xlShiftDown, false);
