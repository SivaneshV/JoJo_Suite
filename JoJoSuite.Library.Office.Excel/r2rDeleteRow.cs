        ...
        public bool DoAction()
        {
            ...
            try
            {
                if (_xlTableName != "")
                {
                    ...
                    for (int i = 1; i <= ListCount; i++)
                    {
                        if (_xlTableName.ToString().ToLower() == _xlWorksheet.ListObjects[i].Name.ToString().ToLower())
                        {
                            ...
                            {
                                _xlWorksheet.ListObjects[i].ListRows.Item[_xlRowNoOrName].Delete();
                            }
                            break;
                        }
                    }
                ...
                else
                {
                    for (int j = 1; j <= _xlCount; j++)
                    {
                        _xlWorksheet.Range[_xlRowNoOrName].EntireColumn.Delete(XlDeleteShiftDirection.xlShiftToLeft);
